...
public class OrderInfoController {
    ...
    @PostMapping("/add")
    public Result add(@Validated(ValidationGroups.OrderCreate.class) @RequestBody OrderRequest orderRequest){
        return orderInfoService.add(orderRequest);
    }

    @ApiOperation("订单详情（扁平，兼容乘客/司机）")
    @GetMapping("/detail")
    public Result<OrderInfo> detail(@RequestParam Long orderId){
        return orderInfoService.detail(orderId);
    }

    @ApiOperation("订单详情（嵌套 order+trip，管理端用）")
    @GetMapping("/detail-nested")
    public Result<OrderNestedVO> detailNested(@RequestParam Long orderId){
        return orderInfoService.detailNested(orderId);
    }

    @ApiOperation("去接乘客并通知")
    @PostMapping("/to-pick-up-passenger")
    public Result toPickUpPassenger(@Validated(ValidationGroups.OrderIdRequired.class) @RequestBody OrderRequest orderRequest){
        return orderInfoService.toPickUpPassenger(orderRequest);
    }

    @ApiOperation("到达乘客出发地,并通知")
    @PostMapping("/arrived-departure")
    public Result<OrderInfo> arrivedDeparture(@Validated(ValidationGroups.OrderIdRequired.class) @RequestBody OrderRequest orderRequest){
        return orderInfoService.arrivedDeparture(orderRequest);
    }

    @ApiOperation("乘客上车司机开始行程,通知前端")
    @PostMapping("/pick-up-passenger")
    public Result<OrderInfo> pickUpPassenger(@Validated(ValidationGroups.OrderIdRequired.class) @RequestBody OrderRequest orderRequest){
        return orderInfoService.pickUpPassenger(orderRequest);
    }

    @ApiOperation("乘客到达目的地,行程终止")
    @PostMapping("/passenger-getoff")
    public Result<OrderInfo> passengerGetOff(@Validated(ValidationGroups.OrderIdRequired.class) @RequestBody OrderRequest orderRequest){
    ...
    @ApiOperation("支付完成")
    @PostMapping("/pay")
    public Result pay(@Validated(ValidationGroups.OrderIdRequired.class) @RequestBody OrderRequest orderRequest){
    ...
    @ApiOperation("乘客/司机查看本单行程轨迹（完成后回放）")
    @GetMapping("/track")
    public Result<OrderTrackVO> orderTrack(@RequestParam Long orderId){
        return orderInfoService.orderTrack(orderId);
    }
