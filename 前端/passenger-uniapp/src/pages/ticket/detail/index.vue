<template>
	<view class="page" v-if="ticket">
		<view class="card card--hero">
			...
			<view class="meta-tag">{{ categoryLabel(PASSENGER_CATEGORIES, ticket.category) }}</view>
			<view class="content">{{ ticket.content }}</view>
			<view v-if="ticket.orderId" class="meta-block">订单 #{{ ticket.orderId }}</view>
...
<script setup>
	...
	import { PASSENGER_CATEGORIES, statusText, categoryLabel } from '../../../utils/ticketDict'
	...
	const handleAppend = async () => {
		...
		busy.value = true
		try {
			const { error } = await ApiTicketAppendMessage({
