...
function resolveMapContainer() {
	...
	// 优先固定 id（与模板一致）
	let el = document.getElementById(MAP_DOM_ID)
	if (el) return el
	// 兼容 uni-app 部分运行时给节点加前缀 / 包一层
	el = document.querySelector('#' + MAP_DOM_ID)
		|| document.querySelector('.xj-map__canvas')
		|| document.querySelector('.xj-map uni-view')
		|| document.querySelector('.xj-map div')
	return el || null
}

export default {
	mounted() {
		// DOM 就绪后再初始化，避免 Map container div not exist
		setTimeout(() => {
			this.ensureMap()
		}, 50)
	},
	methods: {
		...
		ensureMap(retry = 0) {
			...
			if (!el) {
				...
				console.error('地图容器未找到: #' + MAP_DOM_ID)
			...
			loadPromise = AMapLoader.load({
				...
				.then((Amap) => {
					...
					try {
						const scaleBottom = Math.max(12, Math.round((bottomPadding || 160) * 0.55))
