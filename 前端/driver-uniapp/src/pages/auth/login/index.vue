<template>
	<view class="container">
		<view class="logo">讯家出行，一路畅行</view>
		<view class="login">
...
async function handleGetVerifyCode(){
    ...
    if(!HandleApiError(error) && result && !Object.prototype.hasOwnProperty.call(result, 'code')){
		...
		ShowToast((result && result.message) || '验证码发送失败,请检查账号是否注册,若没注册请联系后台人员注册新用户')
		console.log(error);
	}
}
//正常登录
async function handleLogin() {
	...
	if (Object.prototype.hasOwnProperty.call(res.result, 'code')) {
		ShowToast(res.result.message || '登录失败');
		return;
	}
	if (!res.result.accessToken) {
		ShowToast('登录失败：未返回令牌');
		return;
	}
...
// const handleAccount = ()=>{
//     uni.navigateTo({url:'/pages/dev/account'})
// }
</script>
