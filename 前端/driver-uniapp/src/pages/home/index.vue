<template>
	<view class="driver-home">
		...
		<view class="grid-menu">
			<view class="grid-item" @click="goToPage('/pages/map/index')">
				<view class="icon-bg">
					<uni-icons type="location" size="32" color="#318eff" />
				</view>
				<text>地图</text>
			</view>
			<view class="grid-item" @click="goToPage('/pages/finance/money/index')">
				<view class="icon-bg">
					<uni-icons type="wallet" size="32" color="#318eff" />
				</view>
				<text>钱包</text>
			</view>
			<view class="grid-item" @click="goToPage('/pages/order/list/index')">
				<view class="icon-bg">
					<uni-icons type="calendar" size="32" color="#318eff" />
				</view>
				<text>订单</text>
			</view>
			<view class="grid-item" @click="goToPage('/pages/profile/index')">
				<view class="icon-bg">
					<uni-icons type="person" size="32" color="#318eff" />
				</view>
				<text>个人中心</text>
			</view>
		</view>
...
async function loadCityList() {
	...
	if (!error && result) {
		cityListCache = result;
		if ($store.state.point && $store.state.point.code && !$store.state.point.adcode) {
			$store.commit('setPoint', {
				...$store.state.point,
				adcode: resolveAdcode($store.state.point.code)
			});
		}
	}
}

function resolveAdcode(cityCode) {
	if (!cityCode || !cityListCache.length) return '';
	const hit = cityListCache.find(i => {
		const code = i.citycode;
		if (Array.isArray(code)) return code.includes(cityCode) || code.includes(String(cityCode));
		return String(code) === String(cityCode);
	});
	return hit ? hit.adcode : '';
}
		...
		success: async (result) => {
			...
			if (!error) {
				$store.commit('setPoint', {
					...
					adcode: resolveAdcode(result.address?.cityCode),
...
async function getUserProgressOrder() {
	...
	if (!HandleApiError(error) && result != null && !Object.prototype.hasOwnProperty.call(result, 'code')) {
		orderId = result.id;
		uni.redirectTo({ url: `/pages/order/detail/index?orderId=${orderId}` });
	}
}
...
async function handleWorkStatus(status) {
	let adcode = $store.state.point.adcode || resolveAdcode($store.state.point.code);
	if (status === 1 && !adcode) {
		uni.showToast({ title: '定位城市未识别，请稍后重试', icon: 'none' });
		return;
	}
	// 提交出车请求
...
/* 动态时间卡片*/
.time-card {
	background: linear-gradient(135deg, #0b6bcb, #1890ff 55%, #69c0ff);
	border-radius: 48rpx;
	padding: 48rpx 32rpx;
	margin-bottom: 32rpx;
	position: relative;
	overflow: hidden;
	box-shadow: 0 16rpx 32rpx rgba(24, 144, 255, 0.28);
...
.operation {
	...
	.btn {
		...
		&__primary {
			background: linear-gradient(135deg, #40a9ff, #318eff);
			color: #fff;
		}
